...
import 'package:ovoride/data/controller/driver/review/review_controller.dart';
...
class _UserReviewHistoryState extends State<UserReviewHistory> {
  @override
  void initState() {
    Get.put(ReviewRepo(apiClient: Get.find()), tag: 'driver');
    final controller = Get.put(ReviewController(repo: Get.find(tag: 'driver')), tag: 'driver');
    super.initState();
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegionWidget(
      child: GetBuilder<ReviewController>(tag: 'driver', 
        builder: (controller) {
          return Scaffold(
            backgroundColor: MyColor.secondaryScreenBgColor,
            appBar: CustomAppBar(
              title: controller.rider?.getFullName() ?? MyStrings.rider.tr,
            ),
            body: Padding(
                                  ...
                                  RichText(
                                    text: TextSpan(
                                      children: [
                                        ...
                                        TextSpan(
                                          text: ' ${(double.tryParse(controller.rider?.avgRating ?? "0") ?? 0)}'.toCapitalized(),
